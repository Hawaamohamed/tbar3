...
class ChatController extends Controller
{
   ...
   //API for Chat
    ...
    public function pull_new_message(){
        $user= \Tymon\JWTAuth\Facades\JWTAuth::parseToken()->toUser();

        if(\request()->ajax()){
            $mes1=DB::select("select* from messages where from_id=? and to_id=? or from_id=? and to_id=? and read='no' Order By created_at ",[\request('me'),\request('him'),\request('him'),\request("me")]);
            return response()->json(['messages'=> $mes1],200);
        }
    }
}
